    ...
    <div class="dash-bg py-8">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-5">
            ...
            <div class="profile-strip anim-1">
                <div class="px-6 py-7 flex flex-col sm:flex-row items-center sm:items-start gap-5 relative z-10">
                   ...
                   <div class="flex flex-wrap sm:flex-col gap-2 items-center sm:items-stretch justify-center sm:justify-center flex-shrink-0 sm:self-center" style="min-width:140px;">
                        @if ($status !== 'disetujui')
                            <form action="{{ route('admin.verifikasi.approve', $lowongan->id) }}" method="POST" style="display:contents;">
                                @csrf @method('PATCH')
                        ...
                        @endif
                        @if ($canEdit)
                            <a href="{{ route('admin.verifikasi.edit', $lowongan->id) }}"
                        ...
                        @if ($status !== 'ditolak')
                            <button type="button"
                                ...
                                onclick="window.dispatchEvent(new CustomEvent('open-reject-modal', { detail: { url: '{{ route('admin.verifikasi.reject', $lowongan->id) }}', title: '{{ addslashes($lowongan->judul_lowongan) }}' } }))">
            ...
            <div class="anim-4" style="padding-bottom:8px;">
                <a href="{{ route('admin.verifikasi.index') }}" class="btn-back">
