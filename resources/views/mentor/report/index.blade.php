                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select name="intern_id" 
                                    class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition-all">
                        ...
                        <div class="flex items-end space-x-2">
                            <button type="submit" 
                                    class="flex-1 inline-flex items-center justify-center px-4 py-2.5 bg-purple-600 text-white font-semibold rounded-lg hover:bg-purple-700 shadow-md hover:shadow-lg transition-all duration-300">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr class="bg-purple-50">
                                <th class="px-6 py-4 text-left text-xs font-bold text-purple-900 uppercase tracking-wider rounded-tl-lg">Nama</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-purple-900 uppercase tracking-wider">File</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-purple-900 uppercase tracking-wider">Status</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-purple-900 uppercase tracking-wider">Nilai</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-purple-900 uppercase tracking-wider">Revisi</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-purple-900 uppercase tracking-wider">Dikirim</th>
                                <th class="px-6 py-4 text-center text-xs font-bold text-purple-900 uppercase tracking-wider rounded-tr-lg">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            @forelse($reports as $r)
                                <tr class="hover:bg-purple-50 transition-colors duration-150">
                                        ...
                                        <div class="flex items-center">
                                            @if($r->intern->photo_path)
                                                <img src="{{ url('storage/'.$r->intern->photo_path) }}" 
                                                     class="w-8 h-8 rounded-full object-cover border-2 border-purple-200 mr-3" 
                                                     alt="{{ $r->intern->name }}" />
                                            @else
                                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-purple-400 to-pink-500 flex items-center justify-center mr-3">
                                                    <i class="fas fa-user text-white text-xs"></i>
                                                </div>
                                            @endif
                                            <span class="text-sm font-medium text-gray-900">{{ $r->intern->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <a href="{{ route('download', ['path' => $r->file_path]) }}" 
                                           target="_blank" 
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        @if($r->grade)
                                            <div class="inline-flex items-center px-3 py-1 bg-gradient-to-r from-blue-500 to-indigo-600 rounded-lg shadow-sm">
                                                <span class="font-bold text-white text-lg">{{ $r->grade }}</span>
                                                @if($r->score)
                                                    <span class="text-blue-100 text-xs ml-1">({{ $r->score }})</span>
                                                @endif
                                            </div>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex flex-col items-center space-y-2">
                                            ...
                                            <a href="{{ route('mentor.certificates.create', ['intern_id' => $r->intern->id]) }}"
                                               class="inline-flex items-center px-3 py-1.5 bg-purple-600 text-white text-xs font-semibold rounded-lg hover:bg-purple-700 shadow-sm hover:shadow-md transition-all duration-300 w-full justify-center">
                                ...
                                <tr>
                                    <td colspan="7" class="px-6 py-8 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-500">
                                            ...
                                            <p class="text-xs text-gray-400 mt-1">Data akan muncul ketika anak magang submit laporan</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="bg-white rounded-xl shadow-md border border-green-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">{{ $reports->where('status', 'approved')->count() }}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-yellow-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">{{ $reports->where('status', 'pending')->count() }}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-red-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">{{ $reports->where('status', 'rejected')->count() }}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-orange-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">{{ $reports->where('needs_revision', true)->count() }}</h3>
                    </div>
