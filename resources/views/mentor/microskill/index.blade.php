                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="intern_id" 
                                    class="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                        ...
                        <div class="flex items-end space-x-2">
                            <button type="submit" 
                                    class="flex-1 inline-flex items-center justify-center px-4 py-2.5 bg-indigo-600 text-white font-semibold rounded-lg hover:bg-indigo-700 shadow-md hover:shadow-lg transition-all duration-300">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead>
                            <tr class="bg-indigo-50">
                                <th class="px-6 py-4 text-left text-xs font-bold text-indigo-900 uppercase tracking-wider rounded-tl-lg">Nama</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-indigo-900 uppercase tracking-wider">Judul Course</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-indigo-900 uppercase tracking-wider">Waktu Submit</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-indigo-900 uppercase tracking-wider rounded-tr-lg">Bukti Sertifikat</th>
                            </tr>
                        </thead>
                                        ...
                                        <div class="flex items-center">
                                            @if($s->intern->photo_path)
                                                <img src="{{ url('storage/'.$s->intern->photo_path) }}" 
                                                     class="w-8 h-8 rounded-full object-cover border-2 border-indigo-200 mr-3" 
                                                     alt="{{ $s->intern->name }}" />
                                            @else
                                                <div class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center mr-3">
                                                    <i class="fas fa-user text-white text-xs"></i>
                                                </div>
                                            @endif
                                            <span class="text-sm font-medium text-gray-900">{{ $s->intern->name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($s->photo_path)
                                            <div class="relative group">
                                                <img src="{{ url('storage/'.$s->photo_path) }}" 
                                                     alt="Certificate" 
                                                     class="w-16 h-16 object-cover rounded-lg border-2 border-indigo-200 cursor-pointer hover:border-indigo-400 transition-all shadow-sm hover:shadow-md" 
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 py-8 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-500">
                                            ...
                                            <p class="text-xs text-gray-400 mt-1">Data akan muncul ketika anak magang submit course</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
            ...
            <div class="bg-white rounded-xl shadow-md border border-purple-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">{{ $submissions->pluck('intern_id')->unique()->count() }}</h3>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-md border border-blue-100 p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">{{ $submissions->currentPage() }} / {{ $submissions->lastPage() }}</h3>
                    </div>
